...
The texture is the vector layer of the package. There are three options for the user:
   ...
   3. We want a special horizon pattern. Then we can write our own function to create the desired pattern. Useful are the functions that can be created with the prefix basic_ . A useful tool for this are the functions with the prefix basic_ . 
...
point2 <- process_layer(poly2, number = 5, buffer = 3) %>% 
  sf::st_cast("POINT") %>% 
  sf::st_sf(name = sample(process_symbols$name, length(.), replace = TRUE)) %>% 
...
point3 <- process_layer(poly3, number = 7, buffer = 1.5, layer1 = spoint) %>% 
